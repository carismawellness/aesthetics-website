...
// eslint-disable-next-line @typescript-eslint/no-unused-vars -- kept for future use in breadcrumbs / link labels
function Chevron() {
  return (
    <svg width="9" height="12" viewBox="0 0 9 12" fill="none" style={{ marginLeft: "4px" }} aria-hidden="true">
  ...
  return (
    <div style={{ marginTop: "34px" }}>
      ...
      <div className="flex items-center justify-center gap-2" style={{ marginTop: "13px" }}>
        {/* Stars decorative — labelled for screen readers (P1) */}
        <span
          style={{ color: "#f5b50a", fontSize: "15px", letterSpacing: "1px" }}
          aria-label="5 out of 5 stars"
          role="img"
        >
          ★★★★★
        </span>
        <span style={{ fontSize: "13px", color: "var(--label)" }}>4.9/5 from over 500 customer reviews</span>
      </div>
    </div>
  );
}
...
function FaqShare() {
  ...
  return (
    <nav aria-label="Share this answer" className="flex items-center" style={{ gap: "18px", color: "var(--muted)", paddingBottom: "24px" }}>
      {/* Each icon is a link-sized 44px tap target; aria-label describes action (P2) */}
      <button
        type="button"
        aria-label="Share on Facebook (opens in new tab)"
        style={{ background: "none", border: "none", cursor: "pointer", padding: "10px", minWidth: 44, minHeight: 44, display: "inline-flex", alignItems: "center", justifyContent: "center", color: "var(--muted)", borderRadius: 4, outline: "none", transition: "opacity 200ms ease" }}
        onFocus={(e) => { e.currentTarget.style.outline = "2px solid var(--teal-text)"; e.currentTarget.style.outlineOffset = "2px"; }}
        onBlur={(e) => { e.currentTarget.style.outline = "none"; }}
        onMouseEnter={(e) => { e.currentTarget.style.opacity = "0.7"; }}
        onMouseLeave={(e) => { e.currentTarget.style.opacity = "1"; }}
      >
        <svg {...ic} fill="currentColor" aria-hidden="true"><path d="M22 12a10 10 0 1 0-11.6 9.9v-7H7.9V12h2.5V9.8c0-2.5 1.5-3.9 3.8-3.9 1.1 0 2.2.2 2.2.2v2.5h-1.3c-1.2 0-1.6.8-1.6 1.6V12h2.8l-.4 2.9h-2.4v7A10 10 0 0 0 22 12z" /></svg>
      </button>
      <button
        type="button"
        ...
        onMouseLeave={(e) => { e.currentTarget.style.opacity = "1"; }}
      >
        <svg {...ic} fill="currentColor" aria-hidden="true"><path d="M18.9 2H22l-7 8 8.3 12h-6.5l-5-7.4L6 22H3l7.5-8.6L2 2h6.6l4.6 6.8L18.9 2zm-1.1 18h1.7L7.3 4H5.4l12.4 16z" /></svg>
      </button>
      <button
        type="button"
        ...
        onMouseLeave={(e) => { e.currentTarget.style.opacity = "1"; }}
      >
        <svg {...ic} fill="currentColor" aria-hidden="true"><path d="M4.98 3.5A2.5 2.5 0 1 0 5 8.5a2.5 2.5 0 0 0-.02-5zM3 9h4v12H3V9zm6 0h3.8v1.7h.05c.53-1 1.83-2.05 3.77-2.05 4.03 0 4.78 2.65 4.78 6.1V21h-4v-5.4c0-1.3 0-2.95-1.8-2.95-1.8 0-2.08 1.4-2.08 2.86V21H9V9z" /></svg>
      </button>
      <button
        type="button"
        ...
        onMouseLeave={(e) => { e.currentTarget.style.opacity = "1"; }}
      >
        <svg {...ic} fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><path d="M10 13a5 5 0 0 0 7 0l2-2a5 5 0 0 0-7-7l-1 1" /><path d="M14 11a5 5 0 0 0-7 0l-2 2a5 5 0 0 0 7 7l1-1" /></svg>
      </button>
    </nav>
  );
}
...
function StatIcon({ metric }: { metric: string }) {
  ...
  const common = { width: 18, height: 18, viewBox: "0 0 24 24", fill: "none", stroke: "var(--teal-deep)", strokeWidth: 1.5, "aria-hidden": true } as const;
...
function TestimonialCard({ t, index }: { t: { img: string; quote: string; name: string }; index: number }) {
  ...
  return (
    <figure className="card review-card" style={{ overflow: "hidden" }}>
      {/* Use next/image for performance (P3); meaningful alt text (P1) */}
      <Image
        src={t.img}
      ...
      <div style={{ padding: "18px 18px 20px" }}>
        <blockquote id={quoteId} style={{ margin: 0, padding: 0 }}>
          <p style={{ fontSize: "12.5px", color: "var(--label)", lineHeight: 1.6, margin: 0, ...(open ? {} : { display: "-webkit-box", WebkitLineClamp: 4, WebkitBoxOrient: "vertical", overflow: "hidden" }) }}>
            &ldquo;{t.quote}&rdquo;
          </p>
        </blockquote>
        {/* 44px tap target (P2) */}
        <button
          ...
          style={{ fontSize: "12px", color: "var(--teal-text)", marginTop: "10px", background: "none", cursor: "pointer", padding: "8px 0", minHeight: 44, border: "none", display: "inline-flex", alignItems: "center", outline: "none", transition: "opacity 200ms ease" }}
          onFocus={(e) => { e.currentTarget.style.outline = "2px solid var(--teal-text)"; e.currentTarget.style.outlineOffset = "2px"; }}
          onBlur={(e) => { e.currentTarget.style.outline = "none"; }}
          onMouseEnter={(e) => { e.currentTarget.style.opacity = "0.7"; }}
          onMouseLeave={(e) => { e.currentTarget.style.opacity = "1"; }}
        ...
        <figcaption>
          <cite style={{ fontStyle: "normal", display: "block", fontSize: "13px", color: "var(--ink-soft)", marginTop: "12px", fontWeight: 600 }}>
            — {t.name}
          </cite>
        </figcaption>
      </div>
...
export default function PackageFunnel({ data }: { data: PackageData }) {
  ...
  return (
    <>
      ...
      <section aria-label="Package offer" style={{ background: "linear-gradient(180deg, #ffffff 0%, #f7fafa 50%, #ffffff 100%)", padding: "clamp(40px,6vh,72px) 0" }}>
        <div className="container">
          <div className="mx-auto" style={{ maxWidth: "760px", borderRadius: "26px", background: "linear-gradient(160deg,#f7fafa 0%, #fbfdfd 45%, #f7fafa 100%)", border: "1px solid var(--line)", padding: "clamp(24px,3vw,40px)", boxShadow: "0 20px 60px rgba(0,0,0,0.05)" }}>
            <Reveal>
      ...
      {/* ===== TESTIMONIALS ===== */}
      <section aria-label="Customer testimonials" style={{ background: "var(--beige)", padding: "60px 0" }}>
        <div className="container">
      ...
      {/* ===== EMOTIONAL (SO YOUR ...) ===== */}
      {data.emotional && (
        <section aria-label={data.emotional.heading} style={{ padding: "72px 0" }}>
          <div className="container">
            <div className="grid gap-12 lg:grid-cols-2 items-center">
              ...
              <Reveal delay={120}>
                {/* Use next/image (P3); decorative image (no useful alt needed beyond context) */}
                <Image
                  src={data.emotional.image}
                  alt=""
                  role="presentation"
                  width={600}
                  height={600}
                  ...
                  className="w-full"
                  style={{ display: "block", borderRadius: "10px", aspectRatio: "1 / 1", objectFit: "cover", boxShadow: "0 18px 44px rgba(0,0,0,0.10)" }}
                />
              </Reveal>
            </div>
          </div>
      ...
      {data.trusted && (
        <section aria-label="As seen in" style={{ padding: data.emotional ? "20px 0 80px" : "60px 0 80px" }}>
          <div className="container">
            ...
            {/* Press logos — each has meaningful alt text (P1) */}
            <div className="flex flex-wrap items-center justify-center" role="list" aria-label="Media coverage" style={{ gap: "28px", marginTop: "26px" }}>
              {PRESS.map((p) => (
                <div key={p.alt} role="listitem">
                  <Image
                    src={p.src}
                    alt={p.alt}
                    width={120}
      ...
      {/* ===== CREATED FOR WOMEN ===== */}
      {data.createdFor && (
        <section aria-label={data.createdFor.heading} style={{ padding: "20px 0 40px" }}>
          <div className="container">
            ...
            <div className="grid gap-12 lg:grid-cols-2 items-center" style={{ marginTop: "44px" }}>
              <Reveal>
                <ul aria-label="Why this is created for you">
                  {data.createdFor.reasons.map((r) => <Bullet key={r}>{r}</Bullet>)}
                </ul>
                <BookCta href={data.bookHref} save={saveAmt} label={data.ctaBanner} />
              </Reveal>
              <Reveal delay={120}>
                <Image
                  src={data.createdFor.image}
                  alt=""
                  role="presentation"
                  width={600}
                  height={600}
      ...
      {/* ===== YEARS DIVIDER ===== */}
      {data.yearsDivider && (
        <section aria-label="Years of experience" style={{ padding: "30px 0 10px" }}>
      ...
      {/* ===== COMMITMENT / WHY ===== */}
      {data.commitment && (
        <section aria-label="Our commitment" style={{ padding: "30px 0 70px" }}>
          <div className="container">
            <div className="grid gap-12 md:grid-cols-2 mx-auto" style={{ maxWidth: "960px" }}>
              <Reveal>
                <h2 className="font-display" style={{ fontSize: "13px", color: "var(--teal-text)", letterSpacing: "0.14em", marginBottom: "18px" }}>OUR COMMITMENT</h2>
                <ul className="space-y-3" aria-label="Our commitments">
                  {data.commitment.items.map((t) => (<li key={t} className="flex items-start gap-3"><span style={{ color: "var(--teal)", fontSize: "11px", lineHeight: 1.9 }} aria-hidden="true">●</span><span style={{ fontSize: "14.5px", color: "var(--label)", lineHeight: 1.6 }}>{t}</span></li>))}
                </ul>
              </Reveal>
              <Reveal delay={120}>
                <h2 className="font-display" style={{ fontSize: "13px", color: "var(--teal-text)", letterSpacing: "0.14em", marginBottom: "18px" }}>{data.commitment.whyTitle}</h2>
                <ul className="space-y-3" aria-label={data.commitment.whyTitle}>
                  {data.commitment.why.map((t) => (<li key={t} className="flex items-start gap-3"><span style={{ color: "var(--teal)", fontSize: "11px", lineHeight: 1.9 }} aria-hidden="true">●</span><span style={{ fontSize: "14.5px", color: "var(--label)", lineHeight: 1.6 }}>{t}</span></li>))}
                </ul>
              </Reveal>
            </div>
      ...
      {/* ===== OFFER ===== */}
      {data.offer && (
        <section aria-label="Special offer" style={{ background: "var(--beige)", padding: "64px 0" }}>
          <div className="container">
            <div className="grid gap-12 lg:grid-cols-2 items-center">
              <Reveal>
              ...
              <Reveal delay={120}>
                <Image
                  ...
                  sizes="(max-width: 1024px) 90vw, 50vw"
                  className="w-full"
                  style={{ display: "block", borderRadius: "10px", aspectRatio: data.offer.imageRatio ?? "489 / 549", objectFit: "cover", boxShadow: "0 18px 44px rgba(0,0,0,0.12)" }}
                />
              </Reveal>
            </div>
          </div>
      ...
      {/* ===== GET ... BACK ===== */}
      {data.getBack && (
        <section aria-label={data.getBack.heading} style={{ padding: "72px 0" }}>
          <div className="container">
            <div className="grid gap-12 lg:grid-cols-2 items-center">
              <Reveal>
                ...
                <ul style={{ marginTop: "20px" }} aria-label="Benefits">
                  {data.getBack.bullets.map((b) => <Bullet key={b}>{b}</Bullet>)}
                </ul>
                <BookCta href={data.bookHref} save={saveAmt} label={data.ctaBanner} />
              </Reveal>
              <Reveal delay={120}>
                <Image
                  src={data.getBack.image}
                  alt=""
                  role="presentation"
                  width={600}
                  height={600}
      ...
      {data.redefined && (
        <section aria-label={data.redefined.heading} style={{ padding: "20px 0 80px" }}>
          <div className="container">
            <div className="grid gap-12 lg:grid-cols-2 items-center">
              <Reveal>
                <Image
                  src={data.redefined.image}
                  alt=""
                  role="presentation"
                  width={600}
                  height={600}
              ...
              <Reveal delay={120}>
                ...
                <ul style={{ marginTop: "18px" }} aria-label="Treatment highlights">
                  {data.redefined.bullets.map((p) => (<li key={p} className="flex items-start gap-3" style={{ padding: "4px 0" }}><span style={{ color: "var(--muted)", fontSize: "12px", lineHeight: 1.8 }} aria-hidden="true">•</span><span style={{ fontSize: "13.5px", color: "var(--muted)", lineHeight: 1.6 }}>{p}</span></li>))}
                </ul>
                <BookCta href={data.bookHref} save={saveAmt} label={data.ctaBanner} />
              </Reveal>
            </div>
            ...
            {(data.redefined.tabs || data.redefined.stats) && (
              <div className="mx-auto" style={{ maxWidth: "760px", marginTop: "56px" }}>
                {data.redefined.tabs && (
                  // Tab list with proper ARIA role (P1)
                  <div
                    role="tablist"
                    aria-label="Treatment details"
                    className="flex"
                    style={{ gap: "30px", borderBottom: "1px solid var(--line)" }}
                  >
                    {data.redefined.tabs.map((label, i) => (
                      <button
                        ...
                        style={{
                          ...
                          color: tab === i ? "var(--ink)" : "var(--teal-text)",
                ...
                {data.redefined.stats && (
                  <div
                    id={data.redefined.tabs ? `${baseId}-tabpanel-${tab}` : undefined}
                    role={data.redefined.tabs ? "tabpanel" : undefined}
                    aria-labelledby={data.redefined.tabs ? `${baseId}-tab-${tab}` : undefined}
                  ...
                  >
                    {data.redefined.stats.map((s, i) => (
                      <div
                        ...
                        style={{ padding: "14px 0", borderTop: i === 0 ? "none" : "1px solid var(--line)" }}
                      >
                        <span
                          className="shrink-0 inline-flex items-center justify-center"
                          style={{ width: "40px", height: "40px", borderRadius: "50%", border: "1px solid var(--teal-200)" }}
                          aria-hidden="true"
                        >
                          <StatIcon metric={s.metric} />
                        </span>
                        <span style={{ flex: "0 0 200px", fontSize: "14px", color: "var(--ink-soft)", fontWeight: 600 }}>{s.metric}</span>
                        {/* Screen-reader friendly stat value (P1) */}
                        <span style={{ fontSize: "14px", color: "var(--muted)" }} aria-label={`${s.metric}: ${s.value}`}>{s.value}</span>
                      </div>
                    ))}
                  </div>
                )}
      ...
      {/* ===== WHAT TO EXPECT ===== */}
      {data.expect && (
        <section aria-label={data.expect.heading} style={{ padding: "20px 0 80px" }}>
          <div className="container">
              ...
              {data.expect.cols.map((col, i) => (
                <Reveal key={col.label} delay={(i % 3) * 80}>
                  ...
                  <Image
                    ...
                    className="w-full"
                    style={{ display: "block", borderRadius: "8px", aspectRatio: "278 / 221", objectFit: "cover", marginBottom: "16px" }}
                  />
      ...
      {/* ===== FAQ ===== */}
      <section aria-label="Frequently Asked Questions" style={{ padding: "20px 0 70px" }}>
        <div className="container">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4" style={{ marginBottom: "30px" }}>
              ...
              <div className="flex items-center gap-2" style={{ minWidth: "220px" }}>
                ...
                <input
                  ...
                  style={{ flex: 1 }}
                />
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--gold)" strokeWidth="1.6" aria-hidden="true"><circle cx="11" cy="11" r="7" /><path d="M21 21l-4-4" /></svg>
              </div>
            </div>

            {/* FAQ accordion — full ARIA accordion pattern (P1 / spec) */}
            {data.faq.map((f, i) => {
              const faqBtnId = `${baseId}-faq-btn-${i}`;
              const faqPanelId = `${baseId}-faq-panel-${i}`;
              ...
              return (
                <div key={f.q} style={{ borderBottom: "1px solid var(--line)" }}>
                  <h3 style={{ margin: 0 }}>
                    ...
                    >
                      <span style={{ fontSize: "16px", color: "var(--gold)", lineHeight: 1.4 }}>{f.q}</span>
                      <svg
                        width="18"
                        height="18"
                        viewBox="0 0 24 24"
          ...
          <div className={`grid gap-6 ${data.recommended.cards.length === 2 ? "sm:grid-cols-2" : "sm:grid-cols-3"} mx-auto`} style={{ marginTop: "40px", maxWidth: data.recommended.cards.length === 2 ? "620px" : "920px" }}>
            {data.recommended.cards.map((r, i) => (
              <Reveal key={r.label} delay={(i % 3) * 80}>
                <Link
                  href={r.href}
                  className="card review-card block"
                ...
                >
                  <Image
                    ...
                    style={{ display: "block", width: "100%", aspectRatio: "286 / 335", objectFit: "cover" }}
                  />
