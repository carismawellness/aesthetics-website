    ...
    if (d.type === 'BOLD') node = <strong>{node}</strong>;
      ...
      node = (
        <a
          ...
          className="underline text-[var(--teal)] hover:text-[var(--teal-deep)]"
        >
          {node}
        </a>
      );
...
function RenderNode({ node, idx }: { node: WixRichNode; idx: number }) {
  switch (node.type) {
    case 'PARAGRAPH': {
      ...
      return (
        <p key={idx} className="mb-4 text-[#4a4a4a] leading-relaxed text-[16px]" style={{ fontFamily: 'Roboto Local, sans-serif' }}>
    ...
    case 'HEADING': {
      ...
      const base = 'font-semibold mb-3 mt-8 text-[var(--ink)]';
      if (level === 1) return <h1 key={idx} className={`${base} text-3xl`} style={{ fontFamily: 'Trajan Pro, serif' }}>{text}</h1>;
      if (level === 2) return <h2 key={idx} className={`${base} text-2xl`} style={{ fontFamily: 'Trajan Pro, serif' }}>{text}</h2>;
      if (level === 3) return <h3 key={idx} className={`${base} text-xl`} style={{ fontFamily: 'Novecento Wide, sans-serif', letterSpacing: '0.05em' }}>{text}</h3>;
      if (level === 4) return <h4 key={idx} className={`${base} text-lg`} style={{ fontFamily: 'Novecento Wide, sans-serif' }}>{text}</h4>;
      return <h5 key={idx} className={`${base} text-base`}>{text}</h5>;
    }
    ...
    case 'IMAGE': {
      ...
      return (
        <figure key={idx} className="my-8">
          ...
          {caption && <figcaption className="text-center text-sm text-[var(--muted)] mt-2">{caption}</figcaption>}
        </figure>
      );
    }
    ...
    case 'BULLETED_LIST':
      return (
        <ul key={idx} className="list-disc list-outside ml-6 mb-4 space-y-1 text-[#4a4a4a]" style={{ fontFamily: 'Roboto Local, sans-serif' }}>
    ...
    case 'ORDERED_LIST':
      return (
        <ol key={idx} className="list-decimal list-outside ml-6 mb-4 space-y-1 text-[#4a4a4a]" style={{ fontFamily: 'Roboto Local, sans-serif' }}>
          {(node.nodes ?? []).map((item, i) => (
            <li key={i}>{renderTextChildren(item.nodes?.[0]?.nodes ?? item.nodes ?? [])}</li>
          ))}
        </ol>
    ...
    case 'BLOCKQUOTE':
      return (
        <blockquote key={idx} className="border-l-4 border-[var(--teal)] pl-5 my-6 italic text-[var(--muted)]">
          {(node.nodes ?? []).map((n, i) => <RenderNode key={i} node={n} idx={i} />)}
        </blockquote>
      );

    case 'DIVIDER':
      return <hr key={idx} className="my-8 border-[var(--line)]" />;
