...
function DoctorCard({ doctor, index }: { doctor: Doctor; index: number }) {
  ...
  // Aesthetics doctor content shape: name / title / bio[]. Map onto the
  // slimming card's highlight (default) + fullBio (Read more) treatment.
  const highlight = doctor.bio[0];
  const fullBio = doctor.bio.join(' ');
  const hasMore = doctor.bio.length > 1;
