...
/**
 * HeroMotif — "Constellation" (ported from the Carisma Slimming hero, in the
 * aesthetics sage-TEAL colourway).
 *
 * A soft network of dots that slowly float upward and connect to their near
 * neighbours with fading lines — a living lattice that drifts away from the
 * cursor, links stretching and snapping as the dots move. Concentrated in the
 * lower hero and fading up so it clears the copy.
 *
 * 2D canvas + cached radial sprite for the soft dots; direct screen-space mouse
 * interaction (no WebGL). Guards: reduced-motion → one static frame; DPR-aware;
 * paused offscreen / tab hidden; full teardown on unmount.
 *
 * `color` is an "r, g, b" string — default aesthetics teal #96b2b2; the dark
 * pages pass gold.
 */
export default function HeroMotif({ color = '150, 178, 178' }: { color?: string }) {
  ...
  useEffect(() => {
    ...
    // soft dot sprite (radial brand colour → transparent)
    const SP = 64;
    ...
    const draw = (now: number) => {
      ...
      for (let i = 0; i < N; i++) {
        ...
        const x = dot.bx + Math.sin(t * 0.25 + dot.ph) * dot.sway;
        const y = dot.by;
        ...
        X[i] = x + dot.ox;
        Y[i] = y + dot.oy;
        VF[i] = Math.max(0, Math.min(1, (y / H - 0.22) / 0.5));
      }
  ...
  return (
    <div ref={hostRef} aria-hidden="true" style={{ position: 'absolute', inset: 0, zIndex: 0, pointerEvents: 'none' }} />
  );
}
